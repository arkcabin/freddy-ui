    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur-md">
      ...
      <nav className="container relative flex h-16 items-center justify-between px-4 sm:px-6">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {NAV_LINKS.map((link) => (
            <MenuLink key={link.label} label={link.label} hasDropdown={link.hasDropdown} />
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <Button 
            className="hidden h-10 rounded-xl px-6 font-bold shadow-sm transition-all hover:scale-[1.02] active:scale-[0.98] md:flex"
          >
            Get Freddy UI
          </Button>
          ...
          >
            <div className="container flex flex-col gap-6 p-6">
              ...
              <div className="mt-8 flex flex-col gap-3 border-t border-border pt-8">
                <Button variant="outline" className="h-12 w-full rounded-xl text-lg font-bold">
                  Log In
                </Button>
